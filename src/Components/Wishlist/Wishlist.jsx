  ...
  const { data: wishlist = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_SERVER_BASE_URL_Backend}/allWishlist/${session?.user?.email}`
      )
      return res.data
    },
  ...
  const { mutate: deleteWishlist } = useMutation({
    mutationFn: async (id) => {
      const res = await axios.delete(
        `${process.env.NEXT_PUBLIC_SERVER_BASE_URL_Backend}/deleteWishlist/${id}`
      )
      return res.data
    },
    onSuccess: () => {
                ...
                {wishlist.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-6 font-bold text-blue-600 dark:text-blue-400 text-lg">
                      $
                      {parseFloat(item.price?.[locale] || item.price).toFixed(
                        2
                      )}
                    </td>
